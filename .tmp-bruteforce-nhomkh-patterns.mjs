...
const vals=[]
for(let i=0;i<=99;i++){vals.push(`KH${i}`); vals.push(`KH${String(i).padStart(2,'0')}`); vals.push(`N${i}`); vals.push(`G${i}`); vals.push(`GR${i}`); vals.push(`NHOM${i}`); vals.push(`NHOM_${i}`)}
for (const v of vals){
...
console.log('NO_MATCH',vals.length)
