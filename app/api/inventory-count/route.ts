import { NextResponse } from 'next/server'
import { getAuthenticatedClientAndUser, getCurrentSessionProfile } from '@/lib/auth/session'
import { canAccessInventoryCount } from '@/lib/auth/roles'
import {
  executeCreateInventoryCountSheetMutation,
  type CreateInventoryCountSheetBody,
} from '@/lib/inventory-counting/mutations'

export async function POST(request: Request) {
  try {
    const body = (await request.json()) as CreateInventoryCountSheetBody
    const { supabase, user } = await getAuthenticatedClientAndUser()
    const { profile } = await getCurrentSessionProfile()

    if (!canAccessInventoryCount(profile.role)) {
      throw new Error('Chỉ Kiểm kê viên, Thủ kho, KTMH hoặc Admin mới được tạo phiếu kiểm kê.')
    }

    const result = await executeCreateInventoryCountSheetMutation({
      supabase,
      userId: user.id,
      body,
    })

    return NextResponse.json({ ok: true, data: result })
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : error && typeof error === 'object' && 'message' in error
          ? String((error as { message: unknown }).message)
          : 'Không tạo được phiếu kiểm kê.'

    return NextResponse.json({ ok: false, error: message }, { status: 400 })
  }
}
